#!/usr/bin/env python3
"""Generate gutter-cleaning service city data for programmatic SEO."""
import json, os, random

CITIES = [
    # Sun Belt / Suburban — largest residential gutter-cleaning markets
...
def get_climate_note(state, city):
    if state in DESERT:
        return f"{city}'s desert climate demands xeriscaping, irrigation system maintenance, and heat-tolerant plantings. Water conservation gutter-cleaning is both popular and practical"
    elif state in TROPICAL_YEAR_ROUND:
        return f"{city}'s tropical/subtropical climate supports year-round growing — gutter-cleaning companies stay busy every month with fast-growing grass, tropical plantings, and hurricane prep"
    elif state in MILD_WEST:
        return f"{city}'s mild climate extends the growing season. Moss, rain garden management, and drought-tolerant gutter-cleaning are common requests"
    elif state in WARM_SOUTH:
...
def build_providers(city, state):
    ...
    for i in range(5):
        providers.append({
            ...
            "services": ["Lawn Mowing", "Gutter Cleaning Design", "Tree Trimming", "Mulching", "Sprinkler Install"],
            "licensed": True,
            "insured": True,
...
def build_faqs(city, state, costs):
    ...
    return [
        ...
        },
        {
            ...
            "answer": f"In {city}, {get_season(state).lower()} is the active growing season. During peak growth (spring/summer), weekly mowing keeps lawns healthy and prevents scalping. Bi-weekly works during slower growth periods. {note}. Most {city} gutter-cleaning companies offer flexible weekly, bi-weekly, or monthly plans."
        },
        {
            "question": f"What's the best time to do gutter-cleaning in {city}?",
            "answer": f"The best time for major gutter-cleaning projects in {city} is early spring (March–April) or early fall (September–October) when temperatures are moderate and plants establish well. {note}. Booking early in spring is critical — most reputable {city} landscapers are booked 3–6 weeks out by May."
        },
        {
            "question": f"How do I choose a gutter-cleaning company in {city}?",
...
def generate_city(info):
    ...
    return {
        ...
        "gutter-cleaningSeason": get_season(info["stateAbbr"]),
        ...
        "dataSource": "Gutter Cleaning industry cost data, regional market research",
    }
