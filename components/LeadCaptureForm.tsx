...
const SERVICE_TYPES: Record<string, string[]> = {
  treeService: ['Lawn Mowing Service', 'TreeService Design & Install', 'Tree Trimming / Removal', 'Sprinkler System Install', 'Mulch & Cleanup', 'Gutter Cleaning Program (fertilize/weed)', 'Other'],
  ...
  carpetCleaning: ['Steam Cleaning', 'Pet Stain Removal', 'Whole-Home Cleaning', 'Upholstery Cleaning', 'Move-Out Cleaning', 'Area Rug Cleaning', 'Other'],
  applianceRepair: ['Refrigerator Repair', 'Washer / Dryer Repair', 'Dishwasher Repair', 'Oven / Range Repair', 'Ice Maker Repair', 'Premium Brand Service', 'Other'],
}
...
const ACCENT: Record<string, { btn: string; ring: string }> = {
  ...
  carpetCleaning: {
    btn: 'w-full bg-indigo-600 text-white font-bold py-3 rounded-lg hover:bg-indigo-700 transition-colors disabled:opacity-70',
    ring: 'w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500',
  },
  applianceRepair: {
    btn: 'w-full bg-red-600 text-white font-bold py-3 rounded-lg hover:bg-red-700 transition-colors disabled:opacity-70',
    ring: 'w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500',
  },
}
...
export default function LeadCaptureForm({ city, stateAbbr, serviceType = 'treeService' }: { city: string; stateAbbr: string; serviceType?: string }) {
  ...
  if (submitted) return (
    <div className="text-center py-4">
      ...
      <p className="text-sm text-gray-600">Up to 3 {city} carpet cleaning pros will contact you within 24 hours.</p>
    </div>
  )
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3">
      ...
      <textarea placeholder="Any details? Pet stains, rooms, timeline... (optional)" rows={2} className={`${accent.ring} resize-none`} value={form.message} onChange={e => setForm({...form, message: e.target.value})} />
